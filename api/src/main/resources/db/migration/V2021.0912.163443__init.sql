...
CREATE TABLE `classroom` (
    ...
    `no` varchar(32) NOT NULL DEFAULT '' COMMENT '课堂编号',
    `name` varchar(256) NOT NULL COMMENT '课堂名称',
    `user_num` varchar(48) NOT NULL COMMENT '成员数量',
    `created_at` bigint unsigned NOT NULL,
    `updated_at` bigint unsigned NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `idx_no` (`no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci COMMENT='课堂';


CREATE TABLE `classroom_work` (
    `id` bigint unsigned NOT NULL AUTO_INCREMENT,
    `classroom_id` bigint unsigned NOT NULL COMMENT '课堂ID',
